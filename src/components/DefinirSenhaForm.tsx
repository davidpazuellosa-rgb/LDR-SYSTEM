...
// Serve aos dois fluxos: convite (POST /api/convite, com nome) e recuperacao de senha
// (PUT /api/recuperar-senha, sem nome — a conta ja existe).
export default function DefinirSenhaForm({
  token,
  initialName,
  endpoint = "/api/convite",
  method = "POST",
  pedirNome = true,
}: {
  token: string;
  ...
  async function submit() {
    ...
    try {
      const res = await fetch(apiPath(endpoint), {
        ...
        body: JSON.stringify({ token, password, ...(pedirNome ? { name: name.trim() || undefined } : {}) }),
      });
